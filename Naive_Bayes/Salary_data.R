...
#We have dataset which is already partitioned into test and train, the Salary_train data and Salary_Test data. 
#Lets Import the Data
salary_train <- read.csv("E:/Data science Excelr/Assigments/Supervised Machine Learning/Naive Bayes/Salary/SalaryData_train.csv") #This data is used to train the Model
attach(salary_train)

DataExplorer::create_report(salary_train)

salary_test <- read.csv("E:/Data science Excelr/Assigments/Supervised Machine Learning/Naive Bayes/Salary/SalaryData_test.csv") #This data is used to Test the Model
attach(salary_test)
...
#Here there are many variables that we need to convert into an Categorical Variable.
...
#Lets transform the Salary Column into categorical Variable with 2 level high and low.
...
#Lets Standardize our numeric columns
...
#Plots
ggplot(salary_train, aes(x=age, fill = Salary)) + geom_density(alpha = 0.8, color = 'black') + ggtitle("Density Plot") 

ggplot(salary_train) + geom_bar(aes(age)) + ggtitle("People Count with respect to Age") #The data is Skewed at the Right

ggplot(salary_train) + geom_bar(aes(sex), fill = "pink", color = "black") + ggtitle("Male Female Population")

ggplot(salary_train, aes(race)) + geom_histogram(fill = "purple3", stat = "count") #Instance of White People is more than the Other Race people

ggplot(salary_train, aes(age,capitalloss)) + geom_line()
...
conf
acc <-  conf$overall[1]           #Although we can see all of these using conf function still assinging it seperately
...
salary_classifier3 <- naiveBayes(Salary ~ age+education+occupation+educationno+workclass+hoursperweek+native, data = salary_train )
salary_classifier3
...
#The salary_classifier1 model has the best Accuracy ,kappa value,sensitivity and specificity compared to rest of the Models.